...
setInterval(() => {
  const now = Date.now();
  for (const email in otpStorage) { if (otpStorage[email].expires < now) delete otpStorage[email]; }
}, 10 * 60 * 1000);
...
app.post("/api/send-otp", async (req, res) => {
  ...
  otpStorage[sanitizedEmail] = { otp: otpCode, expires: Date.now() + 10 * 60 * 1000 };
...
app.post("/api/verify-otp", (req, res) => {
  ...
  if (record.expires < Date.now()) { delete otpStorage[sanitizedEmail]; return res.status(400).json({ error: "OTP has expired." }); }
...
// ================= TEST ROUTES =================
app.get("/", (req, res) => { res.json({ success: true, message: "Syntrix Referral Backend Operating with Dedicated Queue Architecture" }); });
...
app.post("/api/send-invite", async (req, res) => {
  ...
  if (!friendEmail || !referralCode || !referralLink) { return res.status(400).json({ error: "Missing parameters." }); }
...
app.post("/api/submit-survey", async (req, res) => {
  try {
    ...
    if (isReferralValid && referrerRecord) {
      ...
      await supabase.from("syntrix_referrals").insert([{ referrer_email: referrerRecord.email, referred_email: sanitizedEmail, referral_code: normalizeReferralCode(referredBy), reward_amount: 10, status: "pending", claim_token: claimToken }]);
      await supabase.from("syntrix_rewards").insert([{ email: referrerRecord.email, reward_type: "referral", amount: 10, status: "pending", claim_token: claimToken }]);
...
app.get("/api/user-status", async (req, res) => {
  ...
  try {
    ...
    const { data: userProfile, error } = await supabase.from("syntrix_claims").select("email, status, wallet_address, tx_hash, referral_code, amount_rewarded").eq("email", sanitizedEmail).maybeSingle();
    ...
    if (rewards) { rewards.forEach(r => { if (r.status === "pending" || r.status === "processing") pendingRewards += Number(r.amount); if (r.status === "claimed") claimedRewards += Number(r.amount); }); }
    ...
    return res.json({ success: true, exists: true, isClaimed: isClaimed, status: isClaimed ? "completed" : "verified", referralsCount: totalReferrals || 0, pendingRewards, claimedRewards, referralCode: userProfile.referral_code || null, txHash: userProfile.tx_hash || (queuedItems ? queuedItems.tx_hash : null), walletAddress: userProfile.wallet_address || null });

  } catch (err) { return res.status(500).json({ error: err.message || "Dashboard authentication processing failure" }); }
});
...
app.get("/api/claim-details", async (req, res) => {
  ...
  try {
    const { data: reward, error } = await supabase.from("syntrix_rewards").select("email, amount, reward_type, status").eq("claim_token", token.trim()).maybeSingle();
    if (error || !reward) return res.status(404).json({ error: "Invalid claim token or reward record not found." });
    return res.json({ success: true, email: reward.email, amount: reward.amount, type: reward.reward_type, status: reward.status });
  } catch (err) { return res.status(500).json({ error: "Internal server error reading token properties." }); }
});
...
app.post("/api/execute-claim", async (req, res) => {
  ...
  try {
    ...
    const { data: rewardRecord, error: fetchErr } = await supabase.from("syntrix_rewards").select("id, email, amount, status, reward_type").eq("claim_token", token.trim()).maybeSingle();
    ...
    } catch (sigErr) { return res.status(400).json({ error: "Signature verification processing error: " + sigErr.message }); }
    ...
    await supabase.from("syntrix_payout_queue").insert([{ email: email, wallet_address: sanitizedWallet, reward_amount: Number(rewardRecord.amount), claim_token: token.trim(), status: "queued" }]);
  ...
  } catch (err) { return res.status(500).json({ error: "Fulfillment ingestion failed: " + err.message }); }
});
...
app.post("/api/claim-reward", async (req, res) => {
  ...
  try {
    ...
    await supabase.from("syntrix_payout_queue").insert([{ email: sanitizedEmail, wallet_address: sanitizedWallet, reward_amount: rewardAmount, claim_token: `SURVEY-LAZY-${crypto.randomBytes(8).toString('hex').toUpperCase()}`, status: "queued" }]);
  ...
  } catch (err) { return res.status(500).json({ error: err.message || "Smart contract claim execution pipeline blocked." }); }
});
...
async function processPayoutQueueEngine() {
  ...
  try {
    const { data: queueJob, error } = await supabase.from("syntrix_payout_queue").select("*").eq("status", "queued").order("created_at", { ascending: true }).limit(1).maybeSingle();
    if (error) throw error;
    if (!queueJob) { isQueueProcessing = false; return; }
    ...
    if (!tokenContract) {
      ...
      isQueueProcessing = false; return;
    }
    ...
    try {
      ...
      await supabase.from("syntrix_payout_queue").update({ status: "failed", error_message: blockchainError.message, processed_at: new Date().toISOString() }).eq("id", queueJob.id);
      if (queueJob.claim_token.startsWith("SURVEY-LAZY-")) { await supabase.from("syntrix_claims").update({ status: "pending" }).eq("email", queueJob.email); } 
      else { await supabase.from("syntrix_rewards").update({ status: "pending" }).eq("claim_token", queueJob.claim_token); }
    }
  } catch (engineError) { console.error("[QUEUE ENGINE ENGINE FAILURE CORE CRASH]:", engineError.message); } finally { isQueueProcessing = false; }
}

async function finalizeSuccessfulQueueJob(job, txHash) {
  await supabase.from("syntrix_payout_queue").update({ status: "success", tx_hash: txHash, processed_at: new Date().toISOString() }).eq("id", job.id);
...
function addUniqueThreadGuard() { processPayoutQueueEngine().catch(err => console.error("Thread system leak caught:", err.message)); }
